...
def get_user(scope):
    ...
    user = None
    request = HttpRequest()
    request.META["HTTP_AUTHORIZATION"] = f"Bearer {scope['token']}"
    try:
...
class TokenAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if query_params.get("Authorization"):
            scope["token"] = query_params["Authorization"][0]
            scope["refresh"] = query_params["Refresh"][0]
            scope["user"] = await get_user(scope)
        headers = {header[0].decode(): header[1].decode() for header in scope["headers"]}
        if headers.get("authorization"):
            scope["token"] = headers["authorization"]
            scope["refresh"] = headers["refresh"]
            scope["user"] = await get_user(scope)
